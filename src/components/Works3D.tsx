...
export default function Works3D() {
  ...
  const handleWorkSwitch = (nextVirtualIndex: number) => {
    ...
    if (controlsRef.current) {
      ...
      controlsRef.current.setLookAt(
        offsetX, 10, 10, // Target camera position
  ...
  const resetCamera = () => {
    ...
    if (controlsRef.current) {
      ...
      controlsRef.current.setLookAt(
        offsetX, 10, 10, // High up, looking down
  ...
  return (
    <section id="works-3d" ref={sectionRef} className="relative w-full h-[100svh] bg-[#4D342D] overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none flex flex-col justify-center items-start z-10 p-6 md:p-12">
        ...
        {/* Title */}
        <div className={`absolute top-6 left-16 md:top-12 md:left-24 flex flex-col items-start transition-all duration-350 ease-in-out ${
          isTransitioning ? "opacity-0 -translate-y-2" : "opacity-100 translate-y-0"
        }`}>
          <h2 className="font-serif italic text-3xl md:text-5xl text-[#EDE7DB] tracking-wide flex items-center">
            works 
            <span className="font-sans text-[10px] tracking-[0.4em] uppercase text-[#DDCCB7]/50 not-italic ml-4 align-middle">
              {displayWorkIndex + 1} / {WORKS.length}
            </span>
          </h2>
          <p className="font-sans text-xs md:text-sm tracking-widest text-[#DDCCB7] uppercase mt-2">
            {activeWork.title}
          </p>
        </div>

        {/* Selection Menu */}
        <div className={`absolute right-16 bottom-12 md:right-24 md:bottom-auto md:top-1/2 md:-translate-y-1/2 flex flex-col items-end gap-6 transition-all duration-350 ease-in-out ${
          isTransitioning ? "opacity-0 translate-x-4 pointer-events-none" : "opacity-100 translate-x-0 pointer-events-auto"
        }`}>
          <button
            onClick={resetCamera}
            className={`text-right group flex items-center justify-end gap-4 transition-all duration-500 cursor-pointer ${
              activePoi === null ? "opacity-100 -translate-x-4" : "opacity-40 hover:opacity-80"
            }`}
          >
            <span className="font-sans text-xs md:text-sm tracking-[0.3em] uppercase text-[#EDE7DB]">
              Master Plan
            </span>
            <span className={`h-[1px] transition-all duration-500 ${activePoi === null ? "bg-[#DDCCB7] w-12" : "bg-[#DDCCB7]/30 w-6 group-hover:w-10 group-hover:bg-[#DDCCB7]/60"}`}></span>
          </button>
          ...
          {activeWork.pois.map((poi) => (
            <button
              ...
              className={`text-right group flex items-center justify-end gap-4 transition-all duration-500 cursor-pointer ${
                activePoi === poi.id ? "opacity-100 -translate-x-4" : "opacity-40 hover:opacity-80"
              }`}
            >
              <span className="font-sans text-xs md:text-sm tracking-[0.3em] uppercase text-[#EDE7DB]">
                {poi.title}
              </span>
              <span className={`h-[1px] transition-all duration-500 ${activePoi === poi.id ? "bg-[#DDCCB7] w-12" : "bg-[#DDCCB7]/30 w-6 group-hover:w-10 group-hover:bg-[#DDCCB7]/60"}`}></span>
            </button>
          ))}
        </div>
        ...
        <button 
          ...
          className={`absolute left-4 md:left-6 top-1/2 -translate-y-1/2 pointer-events-auto z-20 w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center bg-[#4D342D]/40 backdrop-blur-sm text-[#DDCCB7]/50 hover:text-[#EDE7DB] hover:scale-105 active:scale-95 transition-all duration-300 ${
        ...
        <button 
          ...
          className={`absolute right-4 md:right-6 top-1/2 -translate-y-1/2 pointer-events-auto z-20 w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center bg-[#4D342D]/40 backdrop-blur-sm text-[#DDCCB7]/50 hover:text-[#EDE7DB] hover:scale-105 active:scale-95 transition-all duration-300 ${
            isTransitioning ? "opacity-20 cursor-not-allowed pointer-events-none" : "cursor-pointer"
          }`}
